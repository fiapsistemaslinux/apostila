...
# Configurando delivery de conteúdo com Criptografia HTTPS

![alt tag](https://raw.githubusercontent.com/wiki/fiap2trc/services/images/mod_ssl.jpg)

O uso do protocolo SSL no apache cria uma configuração que permite trafegar dados sensíveis e confidenciais com segurança através da internet, SSL (Secure Socket Layer) é o protocolo que e encripta toda a transmissão entre o cliente e um servidor de conteúdo, Sites de comércio eletrônico e páginas de autenticação ou que envolvam qualquer outro tipo de informação confidencial utilizam essa camada de segurança.

## Conceitos

Para entender um pouco melhor o funcionamento deste protocolo começemos pelo básico, o uso de certificados digitais:
...
Colocando em linhas gerais e sem detalhamento técnico o processo de validação de um certificado utilizando o protocolo SSL ocorre conforme abaixo:

![alt tag](https://raw.githubusercontent.com/wiki/fiap2trc/services/images/negotiate_ssl.gif)

Informações mais detalhadas sobre esse processo podem ser obtidas [Neste link](https://sites.google.com/site/ddmwsst/digital-certificates#TOC-What-is-a-Digital-Certificate)
...
```sh
apt-get update && apt-get install apache2 curl
```
...
Caso sua instalação tenha sido finalizada com sucesso e caso o acesso a partir da porta 80 esteja OK você já deverá ser capaz de visualizar uma página em CSS com conteúdo default entregue pela instalação, esse conteúdo localiza-se no ***DocumentRoot*** do apache, no Ubuntu trata-se do diretório ***/var/www/html.***
...
O layout de configuração para uma instalação de servidor web Apache2 em sistemas Ubuntu é a seguinte:

![alt tag](https://raw.githubusercontent.com/wiki/fiap2trc/services/images/ubuntu_layout.png)
...
> Um detalhe importante sobre os diretórios "*-enable" é que eles suas configurações ativadas a partir de links simbólicos para outros diretórios de configuração disponíveis a partir de suas respectivas contrapartes como nome "*-available".
> Essas contrapartes devem ser gerenciados usando os comandos a2enmod, a2dismod, a2ensite, a2dissite e a2enconf, a2disconf. 
> Parece complicado mas após o primeiro teste você pega o jeito!
...
Para este exemplo faremos a configuração do Virtual Host para delivery de um [template](https://github.com/fiap2trc/Services/blob/master/Apache/templates/cloud.tar.bz2) utilizando uma implementação 2.4 do apache no ubuntu server, para começar garanta a existencia do template no DocumentRoot do apache em /var/www/html/

No Ubuntu o apache utiliza o diretório /etc/apache2/sites-available como base para criação de virtual hosts, acesse esse diretório e crie um arquivo chamado [001-openstack.conf](https://github.com/fiap2trc/Services/blob/master/Apache/vhosts/001-cloud.conf)
...
```sh
vim /etc/apache2/sites-available/001-openstack.conf
```
...
```sh
a2entise 001-openstack
service apache2 reload
```

Faça um teste acessando o conteúdo de seu site pelo Server Name ou por um dos Server Alias que utilizou ao criar o seu VirtualHost, Não se esqueça, o apache não cria as entradas de DNS para o domínio desejado, logo você terá de fazer isso manualmente adicionando um registro em seu servidor de DNS ( Em nosso ambiente o bind9 hospedado no mesmo host do apache );

---

## Habilitando criptografia no apache2

O suporte ao uso de SSL é um recurso modular nativo do apache bastantdo apenas que seja habilitado, para isso execute:

```sh
# a2enmod ssl
# service apache2 restart
```
Verifique se o protoclo foi realmente habilitado listando os módulos em uso e verificando a porta 443:
...
Primeiro criamos a nossa chave de 2048 bytes para assinar o certifcado:

```sh
# openssl genrsa -out /etc/ssl/private/apache-selfsigned.key 4096
```

Com a chave em “mãos”, crie um certificado, algumas informações serão requeridas, você pode "facilitar" o processo populando o arquivo /etc/ssl/openssl.cnf com o [padrão](https://github.com/fiap2trc/Services/blob/master/Apache/openssl.cnf) salvo no github da disciplina.
...
```sh
# openssl s_client -connect cloud.fiap.site:443
```
...
Se necessário utilizar o modelo disponível no arquivo [001-cloud-hsts.conf](https://github.com/fiap2trc/Services/blob/master/Apache/vhosts/001-cloud-hsts.conf) no repositório do projeto;
...
```sh
curl --verbose --insecure -s -D- https://cloud.fiap.site/ | grep Strict
```
...
* Connection #0 to host cloud.fiap.site left intact
...
![alt tag](https://raw.githubusercontent.com/wiki/fiap2trc/services/images/letsencryptlogo.jpg)
...
```sh
sudo letsencrypt --apache -d devops.fiap.site # Neste exemplo o dominío utilizado foi "devops.fiap.site"
```

- Será necessário que o dominio specificado seja valido e por tanto já tenha DNS totalmente configurado, além disso será necessário prover um endereço de e-mail válido durate a configuração, esse endereço também será usado para atualização e avisos de renovação de certificado;
